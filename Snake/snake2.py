...
def game_over_screen():
    ...
    screen_text("Натисни END за край или R за рестарт.", red, height // 2 + 50)
    pygame.display.update()
    waiting = True
    restart = False
    while waiting:
...
def restart():
    global snake_size, snake_speed, snake_x, snake_y, snake_body, snake_length, apples, apple, food_size
    global food_x, food_y, food_rect, direction, timer
    # snake
    snake_size = 15
    snake_speed = 15
    snake_x = random.randrange(0, width - snake_size, snake_speed)
...
while running:
    ...
    if snake_x < 0 or snake_y < 0 or snake_x >= width or snake_y >= height or [snake_x, snake_y] in snake_body[:-1]:
    ...
    if snake_head.colliderect(food_rect):
        ...
        if food_sound:
            food_sound.play()
        apple = random.randint(0, 10000) % 2
        food_x = random.randrange(0, width - food_size, snake_size)
        food_y = random.randrange(100, height - food_size, snake_size)
        food_rect = pygame.Rect(food_x, food_y, food_size, food_size)
    ...
    if apple == 0:
        ...
        if apple_boostsize:
            screen.blit(apple_boostsize, (food_rect.x, food_rect.y))
        else:
