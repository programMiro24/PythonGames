...
class Snake:
    def __init__(self, x, y, color_1=green, color_2=blue, size=SNAKE_SIZE, length=SNAKE_LENGTH, direction='RIGHT', points = 0):
        self.size = size
...
class Food:
    ...
    def update(self, snake_body):
        ...
        # 0 - нормална(червена), 1 - увеличаваща дължина(зелена), 2 - увеличава 40т. забавяме(зелена)
        while True:
            self.x = random.randrange(0, width - self.size, snake.size)
            self.y = random.randrange(160, height - self.size, snake.size)
...
def color_convert(color, defaut):
    parts = color.split(',')
    if len(parts) != 3:
        print(f"Невалиден цвят!! Ще използвам по подразбиране - {defaut}.")
        return defaut
    try:
        r, g, b = map(int, parts)
        ...
        print(f"Грешка при конвентиране!!! Ще използвам по подразбиране - {defaut}.")
        return defaut
...
text_1 = font2.render("Въведете цветовете за тялото и главата на змията, които желаете "
                     "в конзолата в формата r, g, b", True, white)
screen.blit(text_1, (10, 10))
pygame.display.flip()
color_1 = color_convert(input("Какъв цвят в RGB да бъде тялото на змията - r, g, b? "), green)
color_2 = color_convert(input("Какъв цвят в RGB да бъде главата на змията, ако не е заредена изображението "
                             "на главата - r, g, b? "), blue)
...
def game_over_screen():
    ...
    screen_text(f"Изядените ябълки са: {food.apples}", red, height // 2 + 20)
...
def restart():
    ...
    # snake
    snake = Snake(random.randrange(0, width - 15, 15), random.randrange(100, height - 15, 15), color_1, color_2)
    snake.init_body()
    # food
    food = Food(snake.body)
    # timer
...
is_boosted = False
boosted_limit_time = BOOSTED_TIME # 5 sec boost
speed_boost_start_time = 0
boosted_fps = BOOST_FPS

slow_fps = SLOW_FPS
is_slow = False
slow_limit_time = SLOWED_TIME # 5 sec slow
speed_slow_time = 0
...
while running:
    ...
    if is_boosted:
        if current_time - speed_boost_start_time < boosted_limit_time:
            ...
            is_boosted = False
            fps_speed = min(start_fps + food.apples // 5, max_fps)
    elif is_slow:
    ...
    snake.move()
    if snake.is_dead():
        if high_score < snake.points:
            high_score = snake.points
    ...
    text_5 = font.render(f"Най-добър резултат: {high_score}", True, black)
    screen.blit(text_1, (10, 10))
    screen.blit(text_2, (10, 40))
    screen.blit(text_3, (10, 70))
    screen.blit(text_4, (10, 100))
    screen.blit(text_5, (10, 130))
    pygame.display.flip()
